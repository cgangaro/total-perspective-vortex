...
DATA_SAMPLE_PATH = "C:/Users/rocke/mne_data"
...
def fetch_events(data_filtered, tmin=-1., tmax=4.):
    ...
    channels = data_filtered.info["ch_names"]
    print(f"Channels: {channels}")
    bad_channels = [x for x in channels if x not in good_channels]
    print(f"Bad channels: {bad_channels}")
    data_filtered.drop_channels(bad_channels)
    print(f"Channels after dropping: {data_filtered.info['ch_names']}")
...
def pre_process_data(subjectID, experiments):
    ...
    # print(f"Experiment {experiments} - {X.shape} epochs, labels: {y.shape}, unique labels: {len(np.unique(y))}")
